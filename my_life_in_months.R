...
life_data <- expand_grid(
  ...
  ungroup() %>%
  filter(!(year == 1987 & month_number < 11))
...
life_data <- life_data %>%
  ...
  mutate(era = case_when(
    year_month == "1987,11" ~ "Childhood",
    year_month == "2001,9" ~ "High School",
    year_month == "2005,9" ~ "Undergraduate",
    year_month == "2009,9" ~ "Working at NYU",
    year_month == "2010,9" ~ "MA at Waterloo",
    year_month == "2013,9" ~ "PhD at Rochester",
    year_month == "2018,6" ~ "Insight",
    year_month == "2018,9" ~ "SparkBeyond"
  )) %>%
  fill(era) %>%
...
life_in_months_role_annotations <- life_in_months_initial_annotations +
  ...
  annotate("text", x = 19, y = role_annotations_y - 2.25, label = "(pysch)", family = role_annotations_font_family, size = roles_size * 0.75, colour = "#6eb4e9") +
  ...
  annotate("text", x = 24.25, y = role_annotations_y - 1, label = "(also psych)", family = role_annotations_font_family, size = roles_size * 0.75, colour = "#4ca3e4") +
  annotate("text", x = 27.5, y = role_annotations_y - 1.5, label = "PhD\nCogSci", family = role_annotations_font_family, lineheight = annotation_lineheight - 0.25, size = roles_size, colour = "#4fc58d") +
  ...
  annotate("text", x = 31, y = role_annotations_y - 0.25, label = "Insight", family = role_annotations_font_family, lineheight = annotation_lineheight, size = roles_size, colour = "#00a87d") +
  ...
  annotate("text", x = 33, y = 1.5, label = "Data Scientist\nProduct Lead", family = role_annotations_font_family, lineheight = annotation_lineheight - 0.25, size = roles_size, colour = "#ab97f3") +
...
life_in_months_final <- life_in_months_role_annotations +
  annotate("text", x = 11, y = 13.1, label = "born + raised in ~toronto", family = role_annotations_font_family, size = annotation_base_size, colour = location_colour) +
  ...
  annotate("text", x = 21, y = 14, label = "moved to nyc", family = role_annotations_font_family, size = annotation_base_size, lineheight = annotation_lineheight, hjust = 0.75, colour = location_colour) +
  geom_curve(aes(x = 22.7, xend = 23, y = 13.8, yend = 12.6), curvature = -0.5, arrow = arrow(length = unit(0.0175, "npc")), colour = location_colour) +
  annotate("text", x = 27, y = 14, label = "moved to waterloo", family = role_annotations_font_family, size = annotation_base_size, lineheight = annotation_lineheight, colour = location_colour) +
